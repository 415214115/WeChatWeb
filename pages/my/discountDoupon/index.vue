<template>
	<view class="discountDoupon">
		<view class="navBox">
			<view class="navList" @tap="selectNavFunc(nav.id)" :class="navListId==nav.id?'selectNav':''" v-for="nav in navList" :key="nav.id">{{nav.title}}</view>
		</view>
...
<style scoped>
	...
	.shopName{
		...
		font-size: 500;
		 display: -webkit-box;
		    ...
		    text-overflow: ellipsis; //超出行数部分使用省略号来结尾
		    overflow: hidden;
		    overflow-wrap: break-word; //控制碰到长单词换行
		    white-space: wrap;
	}
	.expireTime{
		color: #666666;
	...
	.stateTwo .money,.stateTwo .shopName,.stateTwo .fullReduction,.stateTwo .expireTime{
		color: #A7A7A7;
	}
</style>
